    ...
    try:
      if all(itertools.imap(lambda pat, arg:
        ...
        with engine.prove('family', 'son_of', context,
          ...
          for x_1 in gen_1:
            assert x_1 is None, \
              "bc_related0.direct_father_son: got unexpected plan from when clause 1"
    ...
    try:
      if all(itertools.imap(lambda pat, arg:
        ...
        with engine.prove(rule.rule_base.root_name, 'father_son', context,
          ...
          for x_1 in gen_1:
            assert x_1 is None, \
              "bc_related0.grand_father_son: got unexpected plan from when clause 1"
            with engine.prove(rule.rule_base.root_name, 'father_son', context,
                              (rule.pattern(3),
                               rule.pattern(0),
                               rule.pattern(2),)) \
              ...
              for x_2 in gen_2:
                assert x_2 is None, \
                  "bc_related0.grand_father_son: got unexpected plan from when clause 2"
                rule.rule_base.num_bc_rule_successes += 1
                yield
        rule.rule_base.num_bc_rule_failures += 1
    finally:
    ...
    try:
      if all(itertools.imap(lambda pat, arg:
        ...
        with engine.prove(rule.rule_base.root_name, 'father_son', context,
          ...
          for x_1 in gen_1:
            assert x_1 is None, \
              "bc_related0.great_grand_father_son: got unexpected plan from when clause 1"
            with engine.prove(rule.rule_base.root_name, 'father_son', context,
                              (rule.pattern(3),
                               rule.pattern(0),
                               rule.pattern(4),)) \
              ...
              for x_2 in gen_2:
                assert x_2 is None, \
                  "bc_related0.great_grand_father_son: got unexpected plan from when clause 2"
                rule.rule_base.num_bc_rule_successes += 1
                yield
        rule.rule_base.num_bc_rule_failures += 1
    finally:
    ...
    try:
      if all(itertools.imap(lambda pat, arg:
        ...
        with engine.prove(rule.rule_base.root_name, 'father_son', context,
          ...
          for x_1 in gen_1:
            assert x_1 is None, \
              "bc_related0.brothers: got unexpected plan from when clause 1"
            with engine.prove(rule.rule_base.root_name, 'father_son', context,
                              (rule.pattern(0),
                               rule.pattern(3),
                               rule.pattern(2),)) \
              ...
              for x_2 in gen_2:
                assert x_2 is None, \
                  "bc_related0.brothers: got unexpected plan from when clause 2"
    ...
    try:
      if all(itertools.imap(lambda pat, arg:
        ...
        with engine.prove(rule.rule_base.root_name, 'brothers', context,
          ...
          for x_1 in gen_1:
            assert x_1 is None, \
              "bc_related0.uncle_nephew: got unexpected plan from when clause 1"
            with engine.prove(rule.rule_base.root_name, 'father_son', context,
                              (rule.pattern(1),
                               rule.pattern(2),
                               rule.pattern(3),)) \
              ...
              for x_2 in gen_2:
                assert x_2 is None, \
                  "bc_related0.uncle_nephew: got unexpected plan from when clause 2"
    ...
    try:
      if all(itertools.imap(lambda pat, arg:
        ...
        with engine.prove(rule.rule_base.root_name, 'uncle_nephew', context,
          ...
          for x_1 in gen_1:
            assert x_1 is None, \
              "bc_related0.cousins: got unexpected plan from when clause 1"
            with engine.prove(rule.rule_base.root_name, 'father_son', context,
                              (rule.pattern(0),
                               rule.pattern(3),
                               rule.pattern(4),)) \
              ...
              for x_2 in gen_2:
                assert x_2 is None, \
                  "bc_related0.cousins: got unexpected plan from when clause 2"
                mark3 = context.mark(True)
...
def populate(engine):
  This_rule_base = engine.get_create('bc_related0')
